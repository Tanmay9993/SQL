...
-- #having -- when we are grouping by data use having
...
-- Exercise 2 
-- Select the customer id and count the number of reserved seats grouped customer
-- for Oct 2017
...
-- #subqueries
-- non-correlated - inner query can run independently of outer query - inner runs first and once
-- correlated - inner query cannot run independently of outer query - inner runs for every row in outer
...
-- here the derived table is given a name - here it is b.
-- derived table is a temporary table 

select screening_id , customer_id,
(select count(seat_id)
from reserved_seat where booking_id = b.id) as count
from bookings b
order by screening_id;

-- Exerceise 1
-- Select the film name and length for all films with a length greater than the
-- average film length
select name , length_min  from films
where length_min >
	( select avg(length_min) from films);

-- Select the maximum number and minimum number if screenings for a particular film
select max(id) , min(id) from
(select film_id , count(id) as id from screenings
group by film_id)a;

-- Select each film name and the number of screenings for the films
select name,
(select count(id) from screenings
where film_id = f.id) as no_screenings
from films f;
...
-- Concatenate
-- Joining data from 2 columns and return into 1 column

select concat(first_name, last_name) as full_name from customers;
select concat(first_name, " " ,last_name) as full_name from customers;

-- Substring
-- substring(string,start,length); start is from 1
select substring("Tanmay",3,3) as sub;
select substring(name,1,4) as short from films;
select substring(name,-1,5) as short from films;
-- negative will start counting from the last character

-- Upper and Lower
-- Changes the case
select upper(name) as upname from films;
select lower(name) as upname from films;


-- Exercise 1
-- Concatenate the names and length from film table
select concat(name," is ", length_min," mins") as film_length from films;

-- Extract customers email from 5th character onwards
select substring(email,5) as sub_email from customers;
...
select substring(name,-3,3) as short from films;
...
-- Date
select date('2018-06-05 07:45:32') as date_;
-- only extracts date
select start_time from screenings;
select date(start_time) as only_date from screenings;
select * from screenings
...
-- Month
select month('2018-06-07 07:45:32') as month_;
-- only extracts month
select month(start_time) as only_month from screenings;
select * from screenings
where month(start_time) = '10';

-- Year
select year('2018-06-07 07:45:32') as year_;
-- only extracts year
select year(start_time) as only_year from screenings;
select * from screenings
where year(start_time) = '2017';


-- Exercise 2
-- Select film id and start time from the screenings table for the date of 20th 
-- october 2017
select film_id , start_time from screenings
where date(start_time) = '2017-10-20';

-- Select all the data from the screenings table for the start time between the
-- 6th and 13th of October 2017
select * from screenings
where date(start_time) between '2017-10-06' and '2017-10-13';

-- Select all the data from the screenings table for october 2017
select * from screenings
where month(start_time) = '10' and
...
-- Challenge 2
-- Which film had most screenings in october 2017
select * from screenings;
select * from films;
select f.name ,count(s.film_id) as no_screenings from screenings s
join films f on f.id = s.film_id    -- joining table to get values from other table
group by s.film_id                  -- group by film_id
order by no_screenings desc         -- ordering in desc order
limit 1;                            -- to get first value

-- Challenge 3
-- How many bookings did the film 'Jigsaw' have in October 2017
select id from films
where name = 'Jigsaw';
select count(*) as no_screenings from bookings 
